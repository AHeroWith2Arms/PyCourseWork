...
def top_transactions(path: str, date: str) -> list[dict]:
    ...
    if len(df) == 0:
        ...
        df_expanse = df[(df["Статус"] == "OK") & (df["Валюта платежа"] == "RUB") & (df["Валюта операции"] != "RUB")][
            ["Дата операции", "Сумма платежа", "Категория", "Описание"]
        ].rename(columns={"Сумма платежа": "Сумма операции"})
        df_operation = df[(df["Статус"] == "OK") & (df["Валюта платежа"] != "RUB") & (df["Валюта операции"] == "RUB")][
            ["Дата операции", "Сумма операции", "Категория", "Описание"]
        ]
        df_operation_2 = df[
            (df["Статус"] == "OK") & (df["Валюта платежа"] == "RUB") & (df["Валюта операции"] == "RUB")
        ][["Дата операции", "Сумма операции", "Категория", "Описание"]]
...
def views(path: str, date: str) -> str:
