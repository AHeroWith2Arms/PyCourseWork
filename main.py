...
def services_option() -> None:
    ...
    try:
        print(
            "Программа: Удалось бы сохранить "
            f'{investment_bank(month_answer,reading_xlsx("data/operations.xlsx").to_dict("records"), int(limit_answer))}'
        )
    except ValueError:
...
def reports_option() -> None:
    ...
    print(
        """Программа: Также напишите дату, по которую мы будем смотреть траты(в целом
        ...
        try:
            print(
                spending_by_category(
                    reading_xlsx("data/operations.xlsx"),
                    category_answer,
                    end_date_answer,
                )
            )
